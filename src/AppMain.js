...
class AppMain extends Core.AppMain {
  /**
   * 处理进程初始化（Master/Worker进程完成初始化时触发）
   * @override
   * @param processId 初始化完成的进程ID。
   * @param launchParams 进程的初始化参数，即Master进程中的process.argv
   */
  onProcessDidInit(processId, launchParams) {
    ...
    if (processId === 'M') {
      ...
      StateCenter.setState(
        APP_STATE_FIELD_KEY_MESSAGE_STRUCTOR,
        messageStructor
      )
      const handlerLoggerCore = this._buildHandlerLoggerCore(
        env,
        baseLoggerConf
      )
      StateCenter.setState(
  ...
  _buildCoreLogger(env, baseLoggerConf) {
    const { level, maxSize, dateFormat, sourcePath, keepDateNum } =
      baseLoggerConf
  ...
  _buildServiceCore(logger, serviceConf) {
    const { port, handlers, middlewares, baseRoutePath } = serviceConf
  ...
  /**
   * 处理子进程退出（Master进程捕获到Worker进程退出时触发）
   * @override
   */
