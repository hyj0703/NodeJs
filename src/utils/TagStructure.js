...
class TagStructure {
  ...
  _parseStructure(scopeData, keyPath, structure) {
    ...
    for (let i = 0; i < structure.length; i += 1) {
      ...
      // 递归总体结构
      props
        ? this._parseStructure(childScopeData, currentKeyPath, props)
        : (childScopeData.type = 'Value')
    }
  }
  ...
  getTagValue(message) {
    ...
    while (tagDatas.length > 0) {
      ...
      // 定位取值上下文
      let context = message
      for (let i = 0; i < preKeyPath.length; i += 1) {
        const key = preKeyPath[i]
        context = context[key] || {}
      }

      // 进行迭代取值
      for (let i = 0; i < readTags.length; i += 1) {
        const { tag, key } = readTags[i]
        const value = context[key]
        value !== undefined && (result[tag] = value)
        // 取值后对tag进行消费
        let tagIndex = -1
        for (let j = 0; j < tagDatas.length; j += 1) {
  ...
  buildMessage(tagValue, options = {}) {
    let result = null
    const { conflictMode = 'normal', defaultValue, defaultTagValue } = options
    ...
    const buildStructure = (context, scopeDatas) => {
      for (let i = 0; i < scopeDatas.length; i += 1) {
        ...
        if (isTagField) {
          const value =
            tagValue[tag] === undefined ? defaultTagValue : tagValue[tag]
